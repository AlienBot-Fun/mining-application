...
// window.serializeQuery = window.deserializeAnyArray = window.serializeAnyArray = window.deserializeAnyObject = window.serializeAnyObject = window.deserializeAnyvarShort = window.deserializeAnyvar = window.serializeAnyvar = window.deserializeAction = window.deserializeActionData = window.serializeAction = window.serializeActionData = window.transactionHeader = window.getTypesFromAbi = window.getType = window.createAbiTypes = window.createInitialTypes = window.hexToUint8Array = window.arrayToHex = window.symbolToString = window.stringToSymbol = window.blockTimestampToDate = window.dateToBlockTimestamp = window.timePointSecToDate = window.dateToTimePointSec = window.timePointToDate = window.dateToTimePoint = window.supportedAbiVersion = window.SerialBuffer = window.SerializerState = void 0;
...
// window.SerializerState = SerializerState;
...
window.supportedAbiVersion = supportedAbiVersion;
...
window.stringToSymbol = stringToSymbol;
/** Convert `Symbol` to `string`. format: `precision,NAME`. */
var symbolToString = function (_a) {
    var name = _a.name, precision = _a.precision;
    return precision + ',' + name;
};
window.symbolToString = symbolToString;
...
var createInitialTypes = function () {
    var result = new Map(Object.entries({
        ...
        float128: createType({
            name: 'float128',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(window.hexToUint8Array(data), 16); },
            deserialize: function (buffer) { return window.arrayToHex(buffer.getUint8Array(16)); },
        }),
        bytes: createType({
            name: 'bytes',
            serialize: function (buffer, data) {
                if (data instanceof Uint8Array || Array.isArray(data)) {
                    ...
                    buffer.pushBytes(window.hexToUint8Array(data));
                }
            },
            deserialize: function (buffer, state) {
                if (state && state.options.bytesAsUint8Array) {
                    ...
                    return window.arrayToHex(buffer.getBytes());
                }
            },
        }),
        string: createType({
        ...
        time_point: createType({
            name: 'time_point',
            serialize: function (buffer, data) { buffer.pushNumberAsUint64(window.dateToTimePoint(data)); },
            deserialize: function (buffer) { return window.timePointToDate(buffer.getUint64AsNumber()); },
        }),
        time_point_sec: createType({
            name: 'time_point_sec',
            serialize: function (buffer, data) { buffer.pushUint32(window.dateToTimePointSec(data)); },
            deserialize: function (buffer) { return window.timePointSecToDate(buffer.getUint32()); },
        }),
        block_timestamp_type: createType({
            name: 'block_timestamp_type',
            serialize: function (buffer, data) { buffer.pushUint32(window.dateToBlockTimestamp(data)); },
            deserialize: function (buffer) { return window.blockTimestampToDate(buffer.getUint32()); },
        }),
        ...
        symbol: createType({
            name: 'symbol',
            serialize: function (buffer, data) { buffer.pushSymbol(window.stringToSymbol(data)); },
            deserialize: function (buffer) { return window.symbolToString(buffer.getSymbol()); },
        }),
        asset: createType({
            name: 'asset',
        ...
        checksum160: createType({
            name: 'checksum160',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(window.hexToUint8Array(data), 20); },
            deserialize: function (buffer) { return window.arrayToHex(buffer.getUint8Array(20)); },
        }),
        checksum256: createType({
            name: 'checksum256',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(window.hexToUint8Array(data), 32); },
            deserialize: function (buffer) { return window.arrayToHex(buffer.getUint8Array(32)); },
        }),
        checksum512: createType({
            name: 'checksum512',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(window.hexToUint8Array(data), 64); },
            deserialize: function (buffer) { return window.arrayToHex(buffer.getUint8Array(64)); },
        }),
...
var createAbiTypes = function () {
    var initialTypes = window.createInitialTypes();
...
window.createAbiTypes = createAbiTypes;
/** Get type from `types` */
var getType = function (types, name) {
    var type = types.get(name);
    if (type && type.aliasOfName) {
        return window.getType(types, type.aliasOfName);
    }
    if (type) {
        return type;
    }
    if (name.endsWith('[]')) {
        return createType({
            name: name,
            arrayOf: window.getType(types, name.substr(0, name.length - 2)),
    ...
    if (name.endsWith('?')) {
        return createType({
            name: name,
            optionalOf: window.getType(types, name.substr(0, name.length - 1)),
    ...
    if (name.endsWith('$')) {
        return createType({
            name: name,
            extensionOf: window.getType(types, name.substr(0, name.length - 1)),
...
var getTypesFromAbi = function (initialTypes, abi) {
    ...
    try {
        for (var types_1 = __values(types), types_1_1 = types_1.next(); !types_1_1.done; types_1_1 = types_1.next()) {
            ...
            if (type.baseName) {
                type.base = window.getType(types, type.baseName);
            }
            try {
                for (var _r = (e_10 = void 0, __values(type.fields)), _s = _r.next(); !_s.done; _s = _r.next()) {
                    var field = _s.value;
                    field.type = window.getType(types, field.typeName);
                }
            }
...
var transactionHeader = function (refBlock, expireSeconds) {
    ...
    return {
        expiration: window.timePointSecToDate(window.dateToTimePointSec(timestamp) + expireSeconds),
        ref_block_num: refBlock.block_num & 0xffff,
        ref_block_prefix: prefix,
    };
};
window.transactionHeader = transactionHeader;
...
var serializeActionData = function (contract, account, name, data, textEncoder, textDecoder) {
    ...
    return window.arrayToHex(buffer.asUint8Array());
};
window.serializeActionData = serializeActionData;
...
var serializeAction = function (contract, account, name, authorization, data, textEncoder, textDecoder) {
    return {
        ...
        data: window.serializeActionData(contract, account, name, data, textEncoder, textDecoder),
    };
};
...
var deserializeActionData = function (contract, account, name, data, textEncoder, textDecoder) {
    ...
    if (typeof data === 'string') {
        data = window.hexToUint8Array(data);
    }
    if (!action) {
        throw new Error("Unknown action " + name + " in contract " + account);
    }
...
window.deserializeActionData = deserializeActionData;
...
var deserializeAction = function (contract, account, name, authorization, data, textEncoder, textDecoder) {
    return {
        ...
        name: name,
        authorization: authorization,
        data: window.deserializeActionData(contract, account, name, data, textEncoder, textDecoder),
    };
};
window.deserializeAction = deserializeAction;
...
window.serializeAnyvar = serializeAnyvar;
...
window.deserializeAnyvar = deserializeAnyvar;
var deserializeAnyvarShort = function (buffer) {
    return window.deserializeAnyvar(buffer, new SerializerState({ useShortForm: true }));
};
window.deserializeAnyvarShort = deserializeAnyvarShort;
var serializeAnyObject = function (buffer, obj) {
    ...
    try {
        for (var entries_2 = __values(entries), entries_2_1 = entries_2.next(); !entries_2_1.done; entries_2_1 = entries_2.next()) {
            ...
            window.serializeAnyvar(buffer, value);
        }
    }
...
window.serializeAnyObject = serializeAnyObject;
var deserializeAnyObject = function (buffer, state) {
    ...
    for (var i = 0; i < len; ++i) {
        ...
        result[key] = window.deserializeAnyvar(buffer, state);
    }
    return result;
};
window.deserializeAnyObject = deserializeAnyObject;
var serializeAnyArray = function (buffer, arr) {
    ...
    try {
        for (var arr_1 = __values(arr), arr_1_1 = arr_1.next(); !arr_1_1.done; arr_1_1 = arr_1.next()) {
            var x = arr_1_1.value;
            window.serializeAnyvar(buffer, x);
        }
    }
...
window.serializeAnyArray = serializeAnyArray;
var deserializeAnyArray = function (buffer, state) {
    var len = buffer.getVaruint32();
    var result = [];
    for (var i = 0; i < len; ++i) {
        result.push(window.deserializeAnyvar(buffer, state));
    }
    return result;
};
window.deserializeAnyArray = deserializeAnyArray;
var addAdditionalTypes = function () {
    ...
    initialTypes.set('any_object', createType({
        name: 'any_object',
        serialize: window.serializeAnyObject,
        deserialize: window.deserializeAnyObject
    }));
    initialTypes.set('any_array', createType({
        name: 'any_array',
        serialize: window.serializeAnyArray,
        deserialize: window.deserializeAnyArray
    }));
    return initialTypes;
};
...
var serializeQuery = function (buffer, query) {
    ...
    if (arg === undefined) {
        ...
        buffer.push(1);
        window.serializeAnyvar(buffer, arg);
    }
    if (filter === undefined) {
        buffer.push(0);
    }
        ...
        try {
            for (var filter_1 = __values(filter), filter_1_1 = filter_1.next(); !filter_1_1.done; filter_1_1 = filter_1.next()) {
                ...
                window.serializeQuery(buffer, q);
            }
        }
...
window.serializeQuery = serializeQuery;
